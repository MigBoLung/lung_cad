...
BOOL
BounceProcess ( char* pName , int nIndex )
{
...
BOOL
RunService ( char* pName , int nArg , char** pArg )
{
...
//////////////////////////////////////////////////////////////////////
// This routine is used to start your service
VOID WINAPI
ServiceMain ( DWORD dwArgc , LPTSTR *lpszArgv )
{
	DWORD   status = 0;
...
ServiceHandler ( DWORD fdwControl )
{
	...
	switch ( fdwControl )
	{
		...
		case SERVICE_CONTROL_SHUTDOWN :
			...
			// terminate all processes started by
			// this service before shutdown
			for ( i = nMaxProcCount - 1 ;
			      i >= 0 ; --i )
				EndProcess ( i );

			if ( !SetServiceStatus ( hServiceStatusHandle ,
						 &serviceStatus ) )
			{
				long nError = GetLastError ();
				char pTemp[121];
		...
		default :
			...
			else
			{
				...
				char pTemp[121];
				sprintf ( pTemp ,
					"Unrecognized opcode %d" , fdwControl );
				WriteLog ( pTemp );
			}
	}
	...
	while ( nCheckProcessSeconds > 0 )
	{
		      ...
		      i < nMaxProcCount ; ++i )
		{
			if ( pProcInfo[i].hProcess )
			{
				...
				GetPrivateProfileString ( pItem ,
					"Restart" , "No" ,
					pRestart , nBufferSize , pInitFile );
				     ...
				     pRestart[0] == '1' )
				{
					...
					else
					{
						...
						char pTemp[121];
						sprintf ( pTemp ,
		"GetExitCodeProcess failed, error code = %d" ,
		nError );
						WriteLog ( pTemp );
					}
				}
...
main ( int argc , char *argv[] )
{
	...
	// initialize variables for .exe,
	// .ini, and .log file names
	...
	{
		// start a worker thread to check
		// for dead programs (and restart if necessary)
		if ( _beginthread ( WorkerProc , 0 , NULL ) == -1 )
		{
			long nError = GetLastError();
			char pTemp[121];
			sprintf ( pTemp ,
				"_beginthread failed , error code = %d" ,
				nError );
			WriteLog ( pTemp );
		}
